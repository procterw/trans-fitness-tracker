...
This repo includes a minimal local web app that supports meal photo upload and nutrition inference via the OpenAI API.
...
Photo uploads currently append to `tracking-data.json.food_events` with:
- `id` (uuid)
...
## OpenAI photo nutrition inference
Implementation: `src/visionNutrition.js`
- Uses the OpenAI Responses API with `input_image` + a strict Zod schema for structured output.
...
- `src/trackingData.js` — reading/writing + rollover-aware date helpers + event totals
- `src/visionNutrition.js` — OpenAI vision nutrition estimation
- `public/` — static UI
...
(The current code implements the photo → `food_events` flow; activity logging + daily rollups can be added next.)

## Next steps (likely)
- Add endpoints/UI for **manual food logging** (quick-add using food definitions).
- Add endpoints/UI for **activity logging** (cardio/strength/mobility/other) and weekly checklist updates.
- Add rollup logic: optionally generate/refresh `food_log` daily summaries from `food_events`.
- Add dashboards: weekly adherence, cardio volume, glute sessions, calorie/macro averages, weight trend.
