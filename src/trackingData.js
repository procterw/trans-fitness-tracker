...
export async function addFoodEvent({
  ...
  let foodLogRow = applyFoodEventToFoodLogInData(data, {
    date,
    nutrients,
    defaultNotes: "Auto-updated from food_events.",
  });
  context.rowByDate = new Map(
    (Array.isArray(data.food_log) ? data.food_log : [])
      .filter((row) => row && isIsoDateString(row.date))
      .map((row) => [row.date, row]),
  );
  context.eventsByDate = groupEventsByDate(data.food_events);

  foodLogRow = refreshAutoFoodLogNoteInData(data, date, { context }) ?? foodLogRow;
...
export async function syncFoodEventsToFoodLog({ date, onlyUnsynced = true }) {
  ...
  if (syncedCount > 0) {
    refreshAutoFoodLogNoteInData(data, date);
  }
